...
//function enableDates(e) {
function enableDates(e) {
    //if (e) $("#dtpScannedDate").datetimepicker({
    //    closeOnDateSelect: !0,
    //    timepicker: !1,
    //    format: "d-M-Y",
    //    onShow: function (e) {
    ...
    //    var t, n = new Date,
    //        i = n.getMonth();
    //    0 == i ? (t = n.getFullYear() - 1, i = 11) : (t = n.getFullYear(), i = n.getMonth() - 1);
    //    var o = t.toString() + "/" + i.toString() + "/" + n.getDate().toString();
...
function choosefiles(e, t, n) {
    ...
    if (document.getElementById('totalPages') != null)
        document.getElementById('totalPages').style.display = "block";
    ...
    var a = t.replace("\\", "sin");
    $("#bigImg").attr("src", "Viewimg.aspx?FilePath=" + a + "&View=1&&Pg=1&Height=650&Width=550&IsEnc=Y"), $("#prev").unbind("click"), $("#prev").click(function () {
        var e = o.val();
        e--, e > 0 && ($("#bigImg").attr("src", "Viewimg.aspx?FilePath=" + a + "&View=1&&Pg=" + e + "&Height=650&Width=550&IsEnc=Y"), o.val(e))
    }), $("#next").unbind("click"), $("#next").click(function () {
        var e = o.val();
        return e++, e <= i.val() ? ($("#bigImg").attr("src", "Viewimg.aspx?FilePath=" + a + "&View=1&&Pg=" + e + "&Height=650&Width=550&IsEnc=Y"), o.val(e), !1) : void 0
    });
}
...
function choosePDFfiles(e, t, n, f) {
    ...
    var a = t.replace("\\", "sin");
    document.getElementById('bigImgPDF').src = t;
...
document.oncontextmenu = function () {
    ...
    if (document.getElementById('hdnSelecteFile').value == "")
        $('#bigImagePDF').hide();
...
function StopLoadOnErrorUploadFile() {
...
function btnsaveclientclick() {
    ...
    if (($("#cboDocumentType :selected").text().toUpperCase() == "LEGAL DOCUMENTS") && ($("#cboDocumentSubType option:selected").text().toUpperCase() == "ADVANCE DIRECTIVE" || $("#cboDocumentSubType option:selected").text().toUpperCase() == "BIRTH PLAN")) {
        ...
        if (path.toUpperCase().indexOf("PDF") > -1) {
            ...
            var result = openModal("frmGenerateLink.aspx", 200, 500, obj, "MessageWindow");
            { sessionStorage.setItem('StartLoading', 'false'); StopLoadFromPatChart(); }
