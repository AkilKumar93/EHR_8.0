...
function OnSuccessMail(response) {
    ...
    // var user = $('#' + lstCtrl.replace('_listDLC', '_txtDLC'))[0].attributes.getNamedItem("UserRole").value;
    if (user.toUpperCase() == "PHYSICIAN" || user.toUpperCase() == "PHYSICIAN ASSISTANT") {
        ...
        //keywrd.text = 'Click here for Add or Update Keywords';
    ...
    if (splitter == "") {
        if (user.toUpperCase() == "PHYSICIAN" || user.toUpperCase() == "PHYSICIAN ASSISTANT") {
          /*  var keywrd = document.createElement("option");
            keywrd.style.fontWeight = 'bolder';
            keywrd.style.textDecoration = 'none';
            keywrd.style.color = 'black';
            keywrd.style.fontStyle = 'normal';
            keywrd.style.cursor = 'default';
            keywrd.disabled = true;
            keywrd.text = 'Comments';
...
function licstboxclick(e) {
    ...
    var txtValue = '';
    //$('.notes :selected').each(function (i, selected) {
    //    txtValue = $(selected).text();
    //});
    ...
    if (document.getElementById(ID).options[index].innerHTML.trim() == "Click here for Add or Update Keywords") {
        ...
        if (document.getElementById(ID).options[index] != undefined)
            var txtValue = document.getElementById(ID).options[index].innerHTML;
            ...
            if (e.indexOf("LastMammogram") != -1) {//BugID:47706
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (result == "showAlert") {
                            //alert('Multiple Mammogram Tests cannot be selected!');
                            return false;
                        }
                        else {
                            if (txtbox == '' && txtbox.indexOf(txtValue) == -1) {
                ...
                if (document.getElementById(ID).selectedOptions[0].attributes.length > 0) {
                    ...
                    if (document.getElementById('txtNotesBMI_txtDLC') != undefined) {
                        ...
                        if (txtboxnew == '' && txtboxnew.indexOf(txtValue) == -1) {
                            ...
                            if (txtboxnew.indexOf(txtValue) == -1) {
        ...
        if (ID1 == "ctmDLCChief_Complaints_txtDLC") {
            ...
            // var HPItxtValue = args.get_item()._element._item._properties._data.value;
