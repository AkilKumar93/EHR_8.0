...
namespace Acurus.Capella.DataAccess.ManagerObjects
{
    ...
    public partial interface IAccountTransactionManager : IManagerBase<AccountTransaction, ulong>
    {            
        ChargeLineItemTransactionDTO GetPatientBalDueUnapplied(ulong MyHumanId);
        AccountTransaction GetAccountTransactionUsingPPIDAndSourceType(ulong BillToPPLineId, string SourceType);
        int UpdateAccountWithoutTransaction(IList<AccountTransaction> UpdateList, ISession MySession, string MacAddress);
        ...
        IList<AccountTransaction> GetAccountTransactionUsingChargeLineItemID(ulong ChargeLineItemID);                        
        int SaveAccountWithoutTransaction(IList<AccountTransaction> InsertList, ISession MySession, string MACAddress);        
        AccountTransaction GetAccTransUsingChargeLineID(ulong ulChargeLineItemID);
        ...
        IList<AccountTransaction> GetTotalbalance(string HumanId, string EncounterId, string BatchId, string[] ChargeLineid);
        ...
        AccountTransaction GetAccountTransactionByPPLineID(ulong PPLineID);
    ...
    public partial class AccountTransactionManager : ManagerBase<AccountTransaction, ulong>, IAccountTransactionManager
    {
        #region Constructors
        ...
        public IList<AccountTransaction> GetAccountTransactionUsingChargeLineItemID(ulong ChargeLineItemID)
        {
            IList<AccountTransaction> ilstAccountTransaction = new List<AccountTransaction>(); 
            using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
            {
                ICriteria crit = iMySession.CreateCriteria(typeof(AccountTransaction)).Add(Expression.Eq("Charge_Line_Item_ID", ChargeLineItemID)).AddOrder(Order.Asc("Id"));
                ilstAccountTransaction= crit.List<AccountTransaction>();
                iMySession.Close();
            }
            return ilstAccountTransaction;
        }

        public int SaveAccountWithoutTransaction(IList<AccountTransaction> InsertList, ISession MySession, string MACAddress)
        {
            int iResult = 0;
            GenerateXml ObjXML = null;
            IList<AccountTransaction> AccTransTemp = null;
            if ((InsertList != null))
            {
        ...
        public AccountTransaction GetAccountTransactionUsingPPIDAndSourceType(ulong BillToPPLineId, string SourceType)
        {
            AccountTransaction objAcc = new AccountTransaction();
            using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
            {
                ICriteria crit = iMySession.CreateCriteria(typeof(AccountTransaction)).Add(Expression.Eq("Bill_To_PP_Line_Item_ID", BillToPPLineId))
                    .Add(Expression.Eq("Source_Type", SourceType)).AddOrder(Order.Asc("Id"));
                if (crit.List<AccountTransaction>().Count > 0)
                    return crit.List<AccountTransaction>()[0];
                iMySession.Close();
            }
            return objAcc;
        }

        public int UpdateAccountWithoutTransaction(IList<AccountTransaction> UpdateList, ISession MySession, string MacAddress)
        {
            int iResult = 0;
            IList<AccountTransaction> AccList = null;
            GenerateXml ObjXML = null;
            if (UpdateList != null)
            {
        ...
        //            IQuery query1 = session.GetISession().GetNamedQuery("Get.ChargeLineIems.From.AccountTransaction");
        ...
        //            IQuery PaidAmount = iMySession.GetNamedQuery("Get.AccountTransaction.PaidAmount");
        ...
        //    objPaymentPosting.AccountTransList = GetAccountTransactionUsingChargeLineItemID(ChargeLineItemID);
        ...
        public decimal GetSumOfAmountAppliedForPatientsInCheck(ulong CarrierId, string PaymentId, ulong BatchId)
        {
            ...
            using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
            {
                object Sum = iMySession.CreateCriteria(typeof(AccountTransaction)).Add(Expression.Eq("Batch_ID", BatchId))
        ...
        public AccountTransaction GetAccTransUsingChargeLineID(ulong ulChargeLineItemID)
        {
            AccountTransaction AccTransRecord = new AccountTransaction();
            using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
            {
                ISQLQuery sql = iMySession.CreateSQLQuery("Select c.* from account_transaction c where c.Charge_Line_Item_ID ='" + ulChargeLineItemID + "' ")
                        .AddEntity("c", typeof(AccountTransaction));

                AccTransRecord = sql.List<AccountTransaction>()[0];
                iMySession.Close();
            }
        ...
        public IList<ChargeLineItemTransactionDTO> GetChargeLineItemsWithAdjust(ulong HumanId)
        {
            ...
            using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
            {
                ...
                for (int i = 0; i < objLst.Count; i++)
                {
                    ...
                    IQuery query1 = iMySession.GetNamedQuery("Get.ChargeLineIems.From.AccountTransaction");
                    query1.SetString(0, objCharge.Charge_Line_Item_Id.ToString());
                    ArrayList arySum;
                    arySum = new ArrayList(query1.List());
                    ...
                    IQuery query2 = iMySession.GetNamedQuery("Get.ChargeLineIemsPaid.From.AccountTransaction");
                    ...
                    IQuery query3 = iMySession.GetNamedQuery("Get.ChargeLineIemsWriteoff.From.AccountTransaction");
        ...
        //public AccountTransactionDTO GetAccountTransactionUsingAccountTransactionID(ulong AccountTransactionID)
        //{
        //    AccountTransactionDTO objAccountDTO = new AccountTransactionDTO();

        //    using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
        //    {
        //        ICriteria crit = iMySession.CreateCriteria(typeof(AccountTransaction)).Add(Expression.Eq("Id", AccountTransactionID)).AddOrder(Order.Asc("Id"));
        //        objAccountDTO.AccountTransList = crit.List<AccountTransaction>();
        ...
        //}       
        public IList<AccountTransaction> GetTotalbalance(string HumanId, string EncounterId, string BatchId, string[] ChargeLineid)
        {

            IList<AccountTransaction> ReturnBalance = new List<AccountTransaction>();
        ...
        public AccountTransaction GetAccountTransactionByPPLineID(ulong PPLineID)
        {
            AccountTransaction AccRecord = new AccountTransaction();
            using (ISession iMySession = NHibernateSessionManager.Instance.CreateISession())
            {
                ICriteria Criteria = iMySession.CreateCriteria(typeof(AccountTransaction)).Add(Expression.Eq("Bill_To_PP_Line_Item_ID", PPLineID));
                AccRecord = Criteria.List<AccountTransaction>()[0];
                iMySession.Close();
            }
            return AccRecord;
        }
        ...
        public IList<AccountTransaction> GetAccTranByPPLineID(ulong PPLineID)
        {
            IList<AccountTransaction> AccTranList = new List<AccountTransaction>();
            ISession iMySession = NHibernateSessionManager.Instance.CreateISession();
            ICriteria Crit = iMySession.CreateCriteria(typeof(AccountTransaction)).Add(Expression.Eq("Bill_To_PP_Line_Item_ID", PPLineID));
            AccTranList = Crit.List<AccountTransaction>();
            return AccTranList;
        }
