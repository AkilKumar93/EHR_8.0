...
function Download() {
    document.getElementById("hdnDownloadbtn").click();
}
...
function print() {
    ...
    //var imgsrc = $("#_imgBig");//.find("img");// for bug id 70171 
    var Imglink = i.src;
...
function ViewPDF() {
    ...
    if (document.getElementById('_plcImgsThumbs') != null)
...
function ChangePg(e) {
    //next Image
    if (parseInt(document.getElementById("PageBox").value) <= parseInt($("#PageLabel")[0].value.replace("/", "").trim()) && e.title.toUpperCase() == "NEXT IMAGE") {
        var ePageBox = $("#PageLabel")[0].value,
            tvalue = $("#PageBox"),
            n = tvalue.val();
        if ($("#PageLabel")[0].value.replace("/", "").trim() != "1" && parseInt(document.getElementById("PageBox").value) < $("#PageLabel")[0].value.replace("/", "").trim()) {
            n++;
            $("#PageBox")[0].value = n;
        }
    }//previous Image
    else if (parseInt(document.getElementById("PageBox").value) <= parseInt($("#PageLabel")[0].value.replace("/", "").trim()) && e.title.toUpperCase() == "PREVIOUS IMAGE") {
        var ePageBox = $("#PageBox"),
           tvalue = ePageBox.val();
        if ($("#PageLabel")[0].value.replace("/", "").trim() != "1" && tvalue != "1") {
            tvalue--;
            $("#PageBox")[0].value = tvalue;
        }
    }
    //To view image and PDF from ExamPhotos
    if ($("#hdnFileNameExam")[0].value != null && $("#hdnFileNameExam")[0].value != "" && $("#hdnFileNameExam")[0].value.split('|||')[$("#PageBox").val() - 1].toUpperCase().indexOf(".PDF") < 0) {
        var FileName = $("#hdnFileNameExam")[0].value.split('|||');
        $("#hdnFileName")[0].value = FileName[$("#PageBox").val() - 1];
        Src = "ViewImg.aspx?View=1&FilePath=" + FileName[$("#PageBox").val() - 1] + "&Pg=1&Height=" + GetBigSrc("Height") + "&Width=" + GetBigSrc("Width");
        SrcBig = "ViewImg.aspx?View=1&FilePath=" + FileName[$("#PageBox").val() - 1] + "&Pg=1&Height=1000&Width=1000";
        SrcRevert = "ViewImg.aspx?View=1&FilePath=" + FileName[$("#PageBox").val() - 1] + "&Pg=1&Height=600&Width=600";
        SrcNavigate = "ViewImg.aspx?View=1&FilePath=" + FileName[$("#PageBox").val() - 1] + "&Height=600&Width=600";
    ...
    else if ($("#hdnFileNameExam")[0].value != null && $("#hdnFileNameExam")[0].value != "" && $("#hdnFileNameExam")[0].value.split('|||')[$("#PageBox").val() - 1].toUpperCase().indexOf(".PDF") >= 0 && document.getElementById("bigImgPDF") != undefined) {
        $('#divrotate').css("display", "none");
        // $("#_plcImgsThumbs").find(".thumbnailactive").removeClass("thumbnailactive").addClass("thumbnail");
        // $("#_plcImgsThumbs").find("#" + e).removeClass("thumbnail").addClass("thumbnailactive");
        $('#bigImgPDF').css("display", "block");
        $('#_imgBig').css("display", "none");
        $('#PDFholder').css("display", "block");
        $('#PDFholder').css("width", "1142px");
        $('#imgholder').css("display", "none");
        var FileName = $("#hdnFileNameExam")[0].value.split('|||');
        //$("#hdnFileName")[0].value = FileName[$("#PageBox").val() - 1];
        $('#bigImgPDF').attr("src", FileName[$("#PageBox").val() - 1]);
    }
    else {
