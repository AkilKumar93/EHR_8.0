...
namespace Acurus.Capella.UI.Extensions
{
    ...
    public class TIF
    {
        ...
        public int PageCount
        {
            get
            {
                if (m_PageCount!=null&&m_PageCount == -1)
        ...
        private int GetPageCount()
        {
            ...
            {
                if (this.m_FilePathName!=null)
                    throw new Exception("Specific file is not present in the location. The file '" + this.m_FilePathName + "' is not available in the location. Please contact System Administrator." + Environment.NewLine + e.Message + ".");
                else
                    throw new Exception("Specific file is not present in the location. Please contact System Administrator." + Environment.NewLine + e.Message + ".");
            ...
            catch (Exception ex)
            {
                ...
                if (this.m_FilePathName != null)
                {
                    ...
                    try
                    {
                        reply = pinger.Send(Path.GetPathRoot(this.m_FilePathName));
                    }
                    catch (Exception exnew)
                    {
                        if (exnew.InnerException != null && exnew.InnerException.Message != null)
                            throw new Exception("The Network Path is not reachable. Please contact System Administrator. The Network Path is: " + Path.GetPathRoot(this.m_FilePathName) + "." + Environment.NewLine + exnew.InnerException.Message + ".");
                        else
                            throw new Exception("The Network Path is not reachable. Please contact System Administrator. The Network Path is: " + Path.GetPathRoot(this.m_FilePathName) + "." + Environment.NewLine + exnew.Message + ".");
                    }
                    if (reply.Status != IPStatus.Success)
                    {
                        throw new Exception("The Ping test for the Network Path is Failed. Please contact System Administrator. The Network Path is: " + Path.GetPathRoot(this.m_FilePathName));
                    }
                    else
                    {
