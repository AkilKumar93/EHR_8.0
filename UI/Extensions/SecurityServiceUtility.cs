...
namespace Acurus.Capella.UI
{
    public class SecurityServiceUtility
    {
            ...
            if (check == true)
            {
                ...
                foreach (Control c in availControls)
                {
                    ...
                    else if (c is Button)
                    {
                        if (permission == "R")
                        {
                            ...
                            if (result == true)
                            {
                                ...
                                webControl.Enabled = false;
                                webControl.Attributes.Remove("onclick");
                                webControl.Attributes.Remove("onClientClick");
                            }
                                                        ...
                                                        if (subct2.Controls[k].GetType().Name == "HtmlGenericControl")
                                                        {
                                                            ...
                                                            if (subct3.Controls.Count > 0)
                                                            {
                                                                for (int l = 0; l < subct3.Controls.Count; l++)
                                                                {
                                                                    ...
                                                                    {
                                                                        subct4 = (System.Web.UI.HtmlControls.HtmlGenericControl)subct3.Controls[l];
                                                                        if (listMenus.Contains(subct4.ID))
                                                                        {
                    ...
                    else if (c is ImageButton)
                    {
                        if (checkUser == false)
                        {
                            if (c.Parent is UI.UserControls.CustomPhrases)
                            {
                                UI.UserControls.CustomPhrases cus = (UI.UserControls.CustomPhrases)c.Parent;
        ...
        {
            bool check = false;
            if (ClientSession.UserPermissionDTO.ListProc != null && ClientSession.UserPermissionDTO.ListProc.Count > 0)
            {
                for (int i = 0; i < ClientSession.UserPermissionDTO.ListProc.Count; i++)
                {
                    if (ClientSession.UserCurrentProcess == ClientSession.UserPermissionDTO.ListProc[i].ToString())
                    {
                        check = true;
                        break;
                    }
        ...
        private bool checkPBACUserRole(int ScreenId)
        {
            ...
            if (ClientSession.PhysicianId == Convert.ToUInt64(ClientSession.FillEncounterandWFObject.EncRecord.Encounter_Provider_ID) && (ClientSession.UserRole == "Physician" || ClientSession.UserRole == "Physician Assistant" || ClientSession.UserRole.ToUpper() == "SCRIBE"))
            {
                result = true;
        ...
        //    return objScnMngr.GetUserPermisssions(userName);
        ...
        public void ApplyPBACPermissions(Page form)
        {
            ...
            if (sUserCurrentProcess == string.Empty)
            {
                ...
                else if (ClientSession.UserRole != null && ClientSession.UserRole.ToUpper().StartsWith("PHYSICIAN") == true)
                {
                    ClientSession.UserCurrentProcess = "PROVIDER_PROCESS";
                }
                else if (ClientSession.UserRole != null && ClientSession.UserRole.ToUpper().StartsWith("SCRIBE") == true)
                {
                    ClientSession.UserCurrentProcess = "SCRIBE_PROCESS";
                }
                else if (ClientSession.UserPermissionDTO != null && ClientSession.UserPermissionDTO.ListProc != null && ClientSession.UserPermissionDTO.ListProc.Contains("MA_PROCESS"))
                {
                    ClientSession.UserCurrentProcess = "MA_PROCESS";
                }
                else if (ClientSession.UserPermissionDTO != null && ClientSession.UserPermissionDTO.ListProc != null && ClientSession.UserPermissionDTO.ListProc.Contains("MA_PROCESS"))
                ...
                else if (ClientSession.UserPermissionDTO != null && ClientSession.UserPermissionDTO.ListProc != null && ClientSession.UserPermissionDTO.ListProc.Contains("TECHNICIAN_PROCESS"))
                {
                    ClientSession.UserCurrentProcess = "TECHNICIAN_PROCESS";//BugID:52846
                }
                else if (ClientSession.UserPermissionDTO != null && ClientSession.UserPermissionDTO.ListProc != null && ClientSession.UserPermissionDTO.ListProc.Contains("FO_PROCESS"))
                {
                    ClientSession.UserCurrentProcess = "MA_PROCESS";
                }
                else
