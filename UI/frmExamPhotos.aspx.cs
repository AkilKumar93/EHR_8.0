    ...
    public partial class frmExamPhotos : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (Request["type"] != null && Request["type"].ToString() == "Result Upload")
            {
                if (UploadImage.UploadedFiles.Count > 0)
                {
                    ...
                    if (!IsCmg)
                    {
                        ...
                        if (Request["type"] != null && Request["type"].ToString() != "Result Upload")
                        {
                            ScriptManager.RegisterStartupScript(this, this.Page.GetType(), "SaveSuccessfully", "SavedSuccessfully();localStorage.setItem('IsSaveCompleted', true);", true);
                        }
                        // 270101
                        btnMove.Enabled = true;
                    }
            ...
            else  //ExamPhoto
            {
                if (UploadImage.UploadedFiles.Count > 0)
                {
                    ...
                    if (file_lst.Count > 0)
                    {
                        ...
                        { Session["ExamList"] = file_exam_lst; }
                        LoadGrid();
                        ScriptManager.RegisterStartupScript(this, this.Page.GetType(), "SaveSuccessfully", "AutoSaveSuccessful();DisplayErrorMessage('270101');localStorage.setItem('IsSaveCompleted', true);", true);
                    }
                }
                else
                {
            ...
            if (hdnFormType.Value.Trim() == "ResultUpload")
                ...
                ScriptManager.RegisterStartupScript(this, this.Page.GetType(), string.Empty, " {sessionStorage.setItem('StartLoading', 'false');StopLoadFromPatChart();}localStorage.setItem('IsSaveCompleted', true);", true);
            #endregion
